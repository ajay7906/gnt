...
exports.createBlogPost = async (req, res) => {
  upload.single('image')(req, res, async (err) => {
    ...
    try {
      const { title, description, option } = req.body; // Capture the input fields
      console.log('Title:', title);
      console.log('Description:', description);
      ...
      if (req.file) {
        console.log('Image file:', req.file);
        imagePath = `uploads/${req.file.filename}`; // Save the relative path
      } else {
        console.log('No image provided');
      }
      ...
      // Save to the database
      const query = 'INSERT INTO blog (title, description, image, `option`) VALUES (?, ?, ?, ?)';
      const values = [title, description, imagePath, option];
